...
在 **⚙ 设置** 中切换主题（Neon、Steam blue、Arcade、Dark gray、High contrast），
并打开 **悬浮窗** - 一个始终置顶的小窗口，可放在 Steam 上方；在那里抽选会在你的
Steam 库中打开选中的游戏。按 **空格键** 抽选。添加到 Steam 库的非 Steam 游戏也会参与抽选。

## 成就

在 **⚙ 设置** 中打开 **显示成就**，即可在抽中的游戏上看到已解锁/总数；勾选
**成就猎人**（在“仅已安装”旁边）还会给出一个随机的未解锁成就让你去完成。它使用你的
令牌；若无效，请在设置中填入 **Steam API 密钥**。游戏的成就统计必须为公开。
